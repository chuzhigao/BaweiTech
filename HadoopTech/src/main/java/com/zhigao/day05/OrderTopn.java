...
public class OrderTopn {
	...
	public static class OrderTopnMapper extends Mapper<LongWritable, Text, OrderBean, NullWritable>{
		...
		protected void map(LongWritable key, Text value,
			...
			String formatFloat =  "^\\d+(\\.\\d+)?$" ;
			if(fields.length== 5 && fields[3].matches(formatFloat)){
	...
	public static void main(String[] args) throws Exception {
		...
		FileInputFormat.setInputPaths(job, new Path("C:\\HadoopTeacher\\data\\day05\\input"));
		FileOutputFormat.setOutputPath(job, new Path("C:\\HadoopTeacher\\data\\day05\\output1"));
